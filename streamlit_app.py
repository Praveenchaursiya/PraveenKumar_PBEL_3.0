...
<style>
    ...
    .sidebar-divider {
        height: 1px;
        background: linear-gradient(90deg, rgba(15, 23, 42, 0.1) 0%, rgba(15, 23, 42, 0.05) 50%, rgba(15, 23, 42, 0.1) 100%);
        margin: 1rem 0;
    }
    .sidebar-recent-item {
...
def main():
    ...
    if token_manager.user_subscribed:
        st.markdown('<div class="pill success-pill">💎 Premium Member - Unlimited Access</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="pill neutral-pill">🎫 Tokens Remaining: {token_manager.get_token_count()}</div>', unsafe_allow_html=True)
